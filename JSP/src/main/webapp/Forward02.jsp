<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
<%	
String A = "다시 입력하세요";

if(request.getParameter("num1") == "" || request.getParameter("num2") == "")
{
	request.setAttribute("off", A );
	
	RequestDispatcher dispatcher = request.getRequestDispatcher("Forward01.jsp");
	dispatcher.forward(request, response);
	
}else{


	int num1 = Integer.parseInt(request.getParameter("num1"));
	int num2 = Integer.parseInt(request.getParameter("num2"));
	
	
	request.setAttribute("ADD", num1+num2);
	request.setAttribute("SUB", num1-num2);
	request.setAttribute("MUL", num1*num2);
	request.setAttribute("DIV", (double)num1/num2);
	
	RequestDispatcher dispatcher = request.getRequestDispatcher("Forward03.jsp");
	dispatcher.forward(request, response);
	

}
	
%>

</body>
</html>